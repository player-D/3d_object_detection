    ...
    def forward(self, mlvl_feats, boxes_2d, cam_intrinsics, cam_extrinsics, temporal_depth_prior=None):
        ...
        for layer_idx, layer in enumerate(self.decoder_layers):
            ...
            if layer_idx < len(self.decoder_layers) - 1 and layer_idx < len(self.refine_branches):
                print(f"Updating reference points at layer {layer_idx}")
                # 预测偏移量
                delta = self.refine_branches[layer_idx](query)

                # 标准 iterative refinement：logit 空间更新 + detach 切断冗余梯度图
                ref_inv = inverse_sigmoid(reference_points)
                reference_points = torch.sigmoid(ref_inv + delta * self.refine_scale).detach()
